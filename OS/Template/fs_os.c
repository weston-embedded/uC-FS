...
{
    /* #### Block until FS lock is acquired. */
...
{
    /* $$$$ Get working directory associated with current task. */
}
#endif
...
void  FS_OS_WorkingDirSet (CPU_CHAR  *p_working_dir,
                           FS_ERR    *p_err)
{
    /* $$$$ Set working directory associated with current task. */
...
{
                            /* $$$$ Delay for specified time in ms  */
}
...
void  FS_OS_DevInit (FS_QTY   dev_cnt,
                     FS_ERR  *p_err)
{
    /* #### Initialize 'dev_cnt' device locks and device access locks. */
...
void  FS_OS_DevLock (FS_ID    dev_id,
                     FS_ERR  *p_err)
{
    /* #### Block until device lock 'dev_id' is acquired. */
...
void  FS_OS_FileInit (FS_QTY   file_cnt,
                      FS_ERR  *p_err)
{
    /* #### Initialize 'file_cnt' file locks. */
...
void  FS_OS_FileAccept (FS_ID    file_id,
                        FS_ERR  *p_err)
{
    /* #### Acquire file lock 'file_id', if available; do NOT block. */
...
void  FS_OS_FileLock (FS_ID    file_id,
                      FS_ERR  *p_err)
{
    /* #### Block until file lock 'file_id' is acquired. */
