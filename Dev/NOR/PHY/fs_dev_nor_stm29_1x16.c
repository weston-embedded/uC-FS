...
*
* Filename : fs_dev_nor_stm29_1x16.c
* Version  : V4.08.01
*********************************************************************************************************
...
#define  FS_DEV_NOR_STM29_1X16_MODULE
...
#include  <lib_mem.h>
#include  "../../../Source/fs.h"
#include  "fs_dev_nor_stm29_1x16.h"
...
const  FS_DEV_NOR_PHY_API  FSDev_NOR_STM29_1x16 = {
    FSDev_NOR_PHY_Open,
    FSDev_NOR_PHY_Close,
    FSDev_NOR_PHY_Rd,
    FSDev_NOR_PHY_Wr,
                                  ...
                                  FS_ERR               *p_err)
{
    ...
    if ((query_info[0x00] != (CPU_INT08U)ASCII_CHAR_LATIN_UPPER_Q) ||               /* Verify ID string.                */
        ...
        FS_TRACE_INFO(("NOR PHY STM29 1x16: Query string wrong: (0x%02X != 0x%02X) OR (0x%02X != 0x%02X) OR (0x%02X != 0x%02X)\r\n", query_info[0x00], ASCII_CHAR_LATIN_UPPER_Q,
    ...
    if (algo != FS_DEV_NOR_PHY_ALGO_AMD) {
        FS_TRACE_INFO(("NOR PHY STM29 1x16: Algo wrong: %04X != %04X.\r\n", algo, FS_DEV_NOR_PHY_ALGO_AMD));
       *p_err = FS_ERR_DEV_IO;
        return;
    }
    ...
    if ((dev_size_log <= 10u) || (dev_size_log >= 32u)) {
        FS_TRACE_INFO(("NOR PHY STM29 1x16: Device size wrong: (%d <= 10) || (%d >= 32).\r\n", dev_size_log, dev_size_log));
       *p_err = FS_ERR_DEV_IO;
        return;
    }
    ...
    if (blk_region_cnt == 0u) {
        FS_TRACE_INFO(("NOR PHY STM29 1x16: Blk region cnt wrong: (%d == 0).\r\n", blk_region_cnt));
       *p_err = FS_ERR_DEV_IO;
        return;
    }
    ...
    if ((mult_size_log == 0u) || (mult_size_log >= 32u)) {
        ...
        if (mult_size > 4u) {
            mult_size = 4u;
                                                                ...
                                                                /* ------------------- SAVE PHY INFO ------------------ */
    FS_TRACE_INFO(("NOR PHY STM29 1x16: Dev size: %d\r\n",     dev_size));
                                ...
                                FS_ERR               *p_err)
{
    ...
    while (cnt_words > 0u) {
        ...
        if (p_phy_data->WrMultSize > 1u) {                          /* If wr mult cmds supp'd ...                       */
            if ((start & (p_phy_data->WrMultSize - 1u)) == 0u) {    /* ... & access align'd   ...                       */
                if (cnt_words >= p_phy_data->WrMultSize) {          /* ... & at least 1 page to wr.                     */
                    cnt_words_page = p_phy_data->WrMultSize / 2u;
                }
            }
        }
        ...
        if (cnt_words_page > 1u) {                                  /* ----------- PGM DBL/QDPL WORD IN DEV ----------- */
            FSDev_NOR_BSP_WrWord_16(p_phy_data->UnitNbr, p_phy_data->AddrBase + (FS_DEV_NOR_PHY_ADDR_UNLOCK1 * 2u), FS_DEV_NOR_PHY_CMD_PGM_DBL);
        ...
        if (ok != DEF_OK) {                                     /* Rtn err if pgm timed out.                            */
           ...
           *p_err = FS_ERR_DEV_TIMEOUT;
            FS_TRACE_DBG(("NOR PHY STM29 1x16: Pgm timeout.\r\n"));
            return;
        }
        ...
        if (err == DEF_YES) {                                   /* Rtn err if pgm'ing err.                              */
            ...
            FSDev_NOR_BSP_WrWord_16(p_phy_data->UnitNbr, p_phy_data->AddrBase, FS_DEV_NOR_PHY_CMD_RD);
           *p_err = FS_ERR_DEV_IO;
            FS_TRACE_DBG(("NOR PHY STM29 1x16: Pgm err.\r\n"));
            return;
        }
                                      ...
                                      FS_ERR               *p_err)
{
    ...
    if (ok != DEF_OK) {                                         /* Rtn err if erase timed out.                          */
        ...
        FSDev_NOR_BSP_WrWord_16(p_phy_data->UnitNbr, p_phy_data->AddrBase, FS_DEV_NOR_PHY_CMD_RD);
       *p_err = FS_ERR_DEV_TIMEOUT;
        FS_TRACE_DBG(("NOR PHY STM29 1x16: Erase timeout.\r\n"));

    } else {
        word = FSDev_NOR_BSP_RdWord_16(p_phy_data->UnitNbr, p_phy_data->AddrBase);
        ...
        if (err == DEF_YES) {                                   /* Rtn err if erase err.                                */
            ...
            FSDev_NOR_BSP_WrWord_16(p_phy_data->UnitNbr, p_phy_data->AddrBase, FS_DEV_NOR_PHY_CMD_RD);
           *p_err = FS_ERR_DEV_IO;
            FS_TRACE_DBG(("NOR PHY STM29 1x16: Erase err.\r\n"));
