...
*
* Filename : fs_dev_nor_stm29_1x08.c
* Version  : V4.08.01
*********************************************************************************************************
...
#define  FS_DEV_NOR_STM29_1X08_MODULE
...
#include  <lib_mem.h>
#include  "../../../Source/fs.h"
#include  "fs_dev_nor_stm29_1x08.h"
...
*                                           COMMAND DEFINES
*
* Note(s) : (1) For a 16-bit capable device used with a 8-bit bus, the following data/address pairs
...
*   ------------------------------------------------------------------------------------------------------
*   | Read/reset     | 1   |  X   = F0  |            |            |            |            |            |
*   | Read/reset     | 3   |  AAA = AA  |  555 = 55  |  X   = F0  |            |            |            |
...
#define  FS_DEV_NOR_PHY_ADDR_UNLOCK1                  0x0AAAu
#define  FS_DEV_NOR_PHY_ADDR_UNLOCK2                  0x0555u
#define  FS_DEV_NOR_PHY_ADDR_CFI_QUERY                0x00AAu
...
#define  FS_DEV_NOR_PHY_CMD_PGM_DBL                     0x50u
#define  FS_DEV_NOR_PHY_CMD_PGM_QDPL                    0x56u
#define  FS_DEV_NOR_PHY_CMD_PGM_OCTPL                   0x8Bu
...
const  FS_DEV_NOR_PHY_API  FSDev_NOR_STM29_1x08 = {
    FSDev_NOR_PHY_Open,
    FSDev_NOR_PHY_Close,
    FSDev_NOR_PHY_Rd,
    FSDev_NOR_PHY_Wr,
                                  ...
                                  FS_ERR               *p_err)
{
    ...
    if (ok != DEF_OK) {                                         /* If HW could not be init'd ...                        */
       *p_err = FS_ERR_DEV_INVALID_UNIT_NBR;                    /* ... rtn err.                                         */
        return;
                                                                ...
                                                                /* ------------------- RD QUERY INFO ------------------ */
                                                                /* Enter query mode.                                    */
    FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase + FS_DEV_NOR_PHY_ADDR_CFI_QUERY, FS_DEV_NOR_PHY_CMD_CFI_QUERY);

    i = 0u;
    while (i < sizeof(query_info)) {                            /* Rd query info.                                       */
        datum_08      = FSDev_NOR_BSP_RdWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase + 0x20u + (i * 2u));
        ...
        i++;
                                                                ...
                                                                /* Reset dev.                                           */
    FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase, FS_DEV_NOR_PHY_CMD_RD);
    ...
    if ((query_info[0x00] != (CPU_INT08U)ASCII_CHAR_LATIN_UPPER_Q) ||               /* Verify ID string.                */
        ...
        FS_TRACE_INFO(("NOR PHY STM29 1x08: Query string wrong: (0x%02X != 0x%02X) OR (0x%02X != 0x%02X) OR (0x%02X != 0x%02X)\r\n", query_info[0x00], ASCII_CHAR_LATIN_UPPER_Q,
    ...
    if (algo != FS_DEV_NOR_PHY_ALGO_AMD) {
        FS_TRACE_INFO(("NOR PHY STM29 1x08: Algo wrong: %04X != %04X.\r\n", algo, FS_DEV_NOR_PHY_ALGO_AMD));
       *p_err = FS_ERR_DEV_IO;
        return;
    }
    ...
    if ((dev_size_log <= 10u) || (dev_size_log >= 32u)) {
        FS_TRACE_INFO(("NOR PHY STM29 1x08: Device size wrong: (%d <= 10) || (%d >= 32).\r\n", dev_size_log, dev_size_log));
       *p_err = FS_ERR_DEV_IO;
        return;
    }
    ...
    if (blk_region_cnt == 0u) {
        FS_TRACE_INFO(("NOR PHY STM29 1x08: Blk region cnt wrong: (%d == 0).\r\n", blk_region_cnt));
       *p_err = FS_ERR_DEV_IO;
        return;
    }
...
#else
    addr_region_start = p_phy_data->AddrBase;
    region_nbr        = p_phy_data->RegionNbr;
    while (region_nbr > 0u) {
        blk_cnt            = (CPU_INT32U)MEM_VAL_GET_INT16U((void *)&query_info[FS_DEV_NOR_PHY_QUERY_OFF_BLKCNT  + (4u * ((blk_region_cnt - 1u) - region_nbr))]);
        blk_cnt           +=  1u;
        blk_size           = (CPU_INT32U)MEM_VAL_GET_INT16U((void *)&query_info[FS_DEV_NOR_PHY_QUERY_OFF_BLKSIZE + (4u * ((blk_region_cnt - 1u) - region_nbr))]);
        blk_size          *=  256u;
    ...
    mult_size_log = query_info[FS_DEV_NOR_PHY_QUERY_OFF_MULTIBYTE_WR_SIZE];         /* Multi-byte wr size.              */
    if ((mult_size_log == 0u) || (mult_size_log >= 32u)) {
        ...
        if (mult_size > 4u) {
            mult_size = 4u;
                                                                ...
                                                                /* ------------------- SAVE PHY INFO ------------------ */
    FS_TRACE_INFO(("NOR PHY STM29 1x08: Dev size: %d\r\n",     dev_size));
                                ...
                                FS_ERR               *p_err)
{
    ...
    FSDev_NOR_BSP_Rd_08( p_phy_data->UnitNbr,                   /* Copy data to buf.                                    */
                         p_dest,
                                ...
                                FS_ERR               *p_err)
{
    ...
    while (cnt_words > 0u) {
        cnt_words_page = 1u;
        if (p_phy_data->WrMultSize > 1u) {                          /* If wr mult cmds supp'd ...                       */
            if ((start & (p_phy_data->WrMultSize - 1u)) == 0u) {    /* ... & access align'd   ...                       */
                if (cnt_words >= p_phy_data->WrMultSize) {          /* ... & at least 1 page to wr.                     */
                    cnt_words_page = p_phy_data->WrMultSize;
                }
            }
        }
        ...
        if (cnt_words_page > 1u) {                                  /* ----------- PGM DBL/QDPL WORD IN DEV ----------- */
            if (cnt_words_page == 2u) {
                FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase + FS_DEV_NOR_PHY_ADDR_UNLOCK1, FS_DEV_NOR_PHY_CMD_PGM_DBL);
            } else {
                FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase + FS_DEV_NOR_PHY_ADDR_UNLOCK1, FS_DEV_NOR_PHY_CMD_PGM_QDPL);
            }
            page_word = 0u;
            while (page_word < cnt_words_page) {
                src_datum = MEM_VAL_GET_INT08U((void *)(p_src_08 + page_word));
                FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase + start + page_word, src_datum);
                page_word++;
            }

            src_datum = MEM_VAL_GET_INT08U((void *)p_src_08);
            ...
            FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase + FS_DEV_NOR_PHY_ADDR_UNLOCK1, FS_DEV_NOR_PHY_DATA_UNLOCK1);
            FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase + FS_DEV_NOR_PHY_ADDR_UNLOCK2, FS_DEV_NOR_PHY_DATA_UNLOCK2);
            FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase + FS_DEV_NOR_PHY_ADDR_UNLOCK1, FS_DEV_NOR_PHY_CMD_PGM);
            FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase + start,                       src_datum);
        }
        ...
        if (ok != DEF_OK) {                                     /* Rtn err if pgm timed out.                            */
           ...
           *p_err = FS_ERR_DEV_TIMEOUT;
            FS_TRACE_DBG(("NOR PHY STM29 1x08: Pgm timeout.\r\n"));
            return;
        }

        word1 = FSDev_NOR_BSP_RdWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase);
        word2 = FSDev_NOR_BSP_RdWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase);
        word3 = word1 & word2;
        err   = DEF_BIT_IS_SET(word3, FS_DEV_NOR_PHY_REG_ERROR);

        if (err == DEF_YES) {                                   /* Rtn err if pgm'ing err.                              */
                                                                /* Reset dev.                                           */
            FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase, FS_DEV_NOR_PHY_CMD_RD);
           *p_err = FS_ERR_DEV_IO;
            FS_TRACE_DBG(("NOR PHY STM29 1x08: Pgm err.\r\n"));
            return;
        }
        ...
        do {                                                    /* Confirm data.                                        */
            dest_datum = FSDev_NOR_BSP_RdWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase + start);
            timeout--;
        } while ((timeout > 0u) && (src_datum != dest_datum));

        p_src_08  += cnt_words_page;
        start     += cnt_words_page;
                                      ...
                                      FS_ERR               *p_err)
{
    ...
    FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase + FS_DEV_NOR_PHY_ADDR_UNLOCK1, FS_DEV_NOR_PHY_DATA_UNLOCK1);
    FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase + FS_DEV_NOR_PHY_ADDR_UNLOCK2, FS_DEV_NOR_PHY_DATA_UNLOCK2);
    FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase + FS_DEV_NOR_PHY_ADDR_UNLOCK1, FS_DEV_NOR_PHY_CMD_ERASE_SETUP);

    FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase + FS_DEV_NOR_PHY_ADDR_UNLOCK1, FS_DEV_NOR_PHY_DATA_UNLOCK1);
    FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase + FS_DEV_NOR_PHY_ADDR_UNLOCK2, FS_DEV_NOR_PHY_DATA_UNLOCK2);
    FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase + start,                       FS_DEV_NOR_PHY_CMD_ERASE_BLK);
    ...
    ok = FSDev_NOR_BSP_WaitWhileBusy(p_phy_data->UnitNbr,
                                     ...
                                     FS_DEV_NOR_PHY_MAX_BLOCK_ERASE_ms * 1000uL);
                                                                ...
                                                                /* -------------------- CHK FOR ERR ------------------- */
    if (ok != DEF_OK) {                                         /* Rtn err if erase timed out.                          */
                                                                /* Reset dev.                                           */
        FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase, FS_DEV_NOR_PHY_CMD_RD);
       *p_err = FS_ERR_DEV_TIMEOUT;
        FS_TRACE_DBG(("NOR PHY STM29 1x08: Erase timeout.\r\n"));

    } else {
        word = FSDev_NOR_BSP_RdWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase);
        err  = DEF_BIT_IS_SET(word, FS_DEV_NOR_PHY_REG_ERROR);

        if (err == DEF_YES) {                                   /* Rtn err if erase err.                                */
                                                                /* Reset dev.                                           */
            FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase, FS_DEV_NOR_PHY_CMD_RD);
           *p_err = FS_ERR_DEV_IO;
            FS_TRACE_DBG(("NOR PHY STM29 1x08: Erase err.\r\n"));
...
static  CPU_BOOLEAN  FSDev_NOR_PHY_PollFnct (FS_DEV_NOR_PHY_DATA  *p_phy_data)
{
    ...
    word1   = FSDev_NOR_BSP_RdWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase);
    word2   = FSDev_NOR_BSP_RdWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase);
    word3   = FSDev_NOR_BSP_RdWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase);
