...
*
* Filename : fs_dev_nor_amd_1x08.c
* Version  : V4.08.01
*********************************************************************************************************
...
#define  FS_DEV_NOR_AMD_1X08_MODULE
...
const  FS_DEV_NOR_PHY_API  FSDev_NOR_AMD_1x08 = {
    FSDev_NOR_PHY_Open,
    FSDev_NOR_PHY_Close,
    FSDev_NOR_PHY_Rd,
    FSDev_NOR_PHY_Wr,
                                  ...
                                  FS_ERR               *p_err)
{
    ...
    if ((query_info[0x00] != (CPU_INT08U)ASCII_CHAR_LATIN_UPPER_Q) ||               /* Verify ID string.                */
        ...
        FS_TRACE_INFO(("NOR PHY AMD 1x08: Query string wrong: (0x%02X != 0x%02X) OR (0x%02X != 0x%02X) OR (0x%02X != 0x%02X)\r\n", query_info[0x00], ASCII_CHAR_LATIN_UPPER_Q,
    ...
    if (algo != FS_DEV_NOR_PHY_ALGO_AMD) {
        FS_TRACE_INFO(("NOR PHY AMD 1x08: Algo wrong: %04X != %04X.\r\n", algo, FS_DEV_NOR_PHY_ALGO_AMD));
       *p_err = FS_ERR_DEV_IO;
        return;
    }
    ...
    if ((dev_size_log <= 10u) || (dev_size_log >= 32u)) {
        FS_TRACE_INFO(("NOR PHY AMD 1x08: Device size wrong: (%d <= 10) || (%d >= 32).\r\n", dev_size_log, dev_size_log));
       *p_err = FS_ERR_DEV_IO;
        return;
    }
    ...
    if (blk_region_cnt == 0u) {
        FS_TRACE_INFO(("NOR PHY AMD 1x08: Blk region cnt wrong: (%d == 0).\r\n", blk_region_cnt));
       *p_err = FS_ERR_DEV_IO;
        return;
    }
    ...
    mult_size = 0u;
                                                                ...
                                                                /* ------------------- SAVE PHY INFO ------------------ */
    FS_TRACE_INFO(("NOR PHY AMD 1x08: Dev size: %d\r\n",     dev_size));
                                ...
                                FS_ERR               *p_err)
{
    ...
    while (cnt_words > 0u) {
        if (p_phy_data->WrMultSize > 1u) {
            cnt_words_page = p_phy_data->WrMultSize - (start & (p_phy_data->WrMultSize - 1u));
        } else {
            cnt_words_page = 1u;
        }
        ...
        if (cnt_words_page > 1u) {                              /* ----------------- PGM WORDS IN DEV ----------------- */
            ...
            FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase + start,                       FS_DEV_NOR_PHY_CMD_PGM_BUF);
            FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase + start,                      (CPU_INT08U)cnt_words_page - 1u);
            ...
            FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase + start,                       FS_DEV_NOR_PHY_CMD_PGM_BUF_CONFIRM);
        ...
        if (ok != DEF_OK) {                                     /* Rtn err if pgm timed out.                            */
           ...
           *p_err = FS_ERR_DEV_TIMEOUT;
            FS_TRACE_DBG(("NOR PHY AMD 1x08: Pgm timeout.\r\n"));
            return;
        }
        ...
        if (err == DEF_YES) {                                   /* Rtn err if pgm'ing err.                              */
            ...
            FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase, FS_DEV_NOR_PHY_CMD_RD);
           *p_err = FS_ERR_DEV_IO;
            FS_TRACE_DBG(("NOR PHY AMD 1x08: Pgm err.\r\n"));
            return;
        }
                                      ...
                                      FS_ERR               *p_err)
{
    ...
    if (ok != DEF_OK) {                                         /* Rtn err if erase timed out.                          */
        ...
        FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase, FS_DEV_NOR_PHY_CMD_RD);
       *p_err = FS_ERR_DEV_TIMEOUT;
        FS_TRACE_DBG(("NOR PHY AMD 1x08: Erase timeout.\r\n"));
        ...
        if (err == DEF_YES) {                                   /* Rtn err if erase err.                                */
            ...
            FSDev_NOR_BSP_WrWord_08(p_phy_data->UnitNbr, p_phy_data->AddrBase, FS_DEV_NOR_PHY_CMD_RD);
           *p_err = FS_ERR_DEV_IO;
            FS_TRACE_DBG(("NOR PHY AMD 1x08: Erase err.\r\n"));
